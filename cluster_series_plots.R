...
#for(i in 1:6){
#  de$vals$Km[[i+1]] <- de$vals$Km$full[,(6*i-5):(6*i)]
#  de$vals$Rm[[i+1]] <- de$vals$Rm$full[,(6*i-5):(6*i)]
#  de$vals$Rdm[[i+1]] <- de$vals$Rdm$full[,(6*i-5):(6*i)]
#  de$vals$Rddm[[i+1]] <- de$vals$Rddm$full[,(6*i-5):(6*i)]
#  de$vals$csep[[i+1]] <- de$vals$csep$full[,(6*i-5):(6*i)]
#}
#names(de$vals$Km) <- c("full","1","2","3","4","5","6")
#names(de$vals$Rm) <- c("full","1","2","3","4","5","6")
#names(de$vals$Rdm) <- c("full","1","2","3","4","5","6")
#names(de$vals$Rddm) <- c("full","1","2","3","4","5","6")
#names(de$vals$csep) <- c("full","1","2","3","4","5","6")
...
for(i in 1:4){
  plot(de$eff, de$mean[[i]],
       ylim=range(c(de$mean[[i]]-de$sd[[i]], de$mean[[i]]+de$sd[[i]])),
       pch=19, xlab="Blur SD % of Radius", ylab=ylabs[i],
       cex.lab = 1.75, cex.axis = 1.25
  )
  # hack: we draw arrows but with very special "arrowheads"
  arrows(de$eff[de$sd[[i]] != 0], de$mean[[i]][de$sd[[i]] != 0]-de$sd[[i]][de$sd[[i]] != 0], de$eff[de$sd[[i]] != 0], 
...
de$vals$Km <- fread("~/Research/K_cluster_series/190121_denrb/Km.csv")
de$vals$Rm <- fread("~/Research/K_cluster_series/190121_denrb/Rm.csv")
de$vals$Rdm <- fread("~/Research/K_cluster_series/190121_denrb/Rdm.csv")
de$vals$Rddm <- fread("~/Research/K_cluster_series/190121_denrb/Rddm.csv")
...
for(k in 1:4){
  for(i in 1:4){
    ...
    if (i==1){
      ...
      #text(1,1,"99.9% AI RRL Envelope width",pos=2)
      legend(2,60,legend = c("1","0.4","0.2","0.1"), col = color, pch = 19, lwd = 1.3,bty = "n",cex = 1.1,y.intersp= 0.9)
