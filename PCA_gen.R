...
if(nrand != 0){
  set.seed(s)
  rr <- runif(nrand, min = 2, max = 6.5)
  denr <- runif(nrand, min = 0.15, max = 1) 
  rbpr <- runif(nrand, min = 0, max = 0.5)
...
outfin <- matrix(NA, nrow = length(tot), ncol = 16)
for(i in 1:length(tot)){
  outfin[i,1:4] <- tot[[i]]  
}

print("Randoms created...")
print(paste("Total to test: ",toString(length(tot))))
...
t1 <- Sys.time()
outtemp <- parLapply(cl,1:fin, kseries2, p ,tot, maxr, nr, toSub, FALSE, s)
t2 <- Sys.time()
print(t2-t1)
...
outfin[,5:10] <- apply(simplify2array(outtemp), 1:2, mean, na.rm = TRUE)[,1:6]
outfin[,11:16] <- apply(simplify2array(outtemp), 1:2, sd, na.rm = TRUE)[,1:6]
